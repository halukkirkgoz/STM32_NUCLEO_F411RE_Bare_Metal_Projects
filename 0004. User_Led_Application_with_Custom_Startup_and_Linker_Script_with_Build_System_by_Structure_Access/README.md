# STM32F411RE Bare-Metal LED Application with Structure-Based Register Access

This project demonstrates how to build and flash a bare-metal user LED application for the **NUCLEO-F411RE** board. The application controls the on-board LD2 LED connected to **PA5** through memory-mapped peripheral structures, without STM32CubeIDE, STM32 HAL, or CMSIS device headers.
...
* `main.c`: Defines typed RCC and GPIO register structures, maps them to the peripheral base addresses, and uses them to configure PA5 and control the on-board LED.
